...
var loadfields = function(jsonurl) {
	var usedfields;
	var page = '';
	var devs;
	var comps;

	// find out if we are dealing with a game, company or developer page
	...
	$.getJSON(jsonurl, function(data) {
		jQuery.each(data, function(i, val) {
			if ($.inArray(i,['platforms','modes','media', 'genres', 'tags']) >= 0) {
				...
				var str = val.map(function(element) {return element.name;}).join(',');
...
var addField = function(button_element, types) {
	...
	if (types.indexOf('only_label') == 0 ) {
		...
		html += '<option value="remove">Remove</option><option selected="" value="">Select Type</option></select></div>';
		$(button_element).parent().find('.newFieldsDiv').append(html);
...
var addConcreteField = function(params){
	...
	if (field_name == 'remove') { // remove field
		...
		html += addDateInput('release_date' + anzDateInputs);
		html += '<input id="text_release_date' + anzDateInputs + '" name="input_field_name' + anzDateInputs + '" type="text">';
		html += '<button type="button" onclick="addConcreteField({selectedElement: this, deleteCurrentField: false});" value="release dates"> + </button></div>';
		...
		$.getJSON('/ajax.json?type=all&field=' + field_name, function(data) {
			...
			$('#'+input_field_name + '_input').focus(function() {
				...
				$('textarea, input:not(#' + input_field_name + '_input)').bind('focusin.allavailable click.allavailable', function(e) {
					if ($(e.target).closest('#' + input_field_name + '_input').length) {
						return;
					} 
					div.fadeOut('medium');
		...
		html += '<input class="'+field_name+'_hidden" name="'+field_name+'_hidden" type="hidden">';
		html += '<input class="'+field_name+'_link mixin" name="'+field_name+'_link" type="text">';
		html += '<input class="'+field_name+'_text mixin" name="'+field_name+'_text" type="text">';
		html += '<button type="button" onclick="addConcreteField({selectedElement: this, deleteCurrentField: false});" value="'+field_name+'"> + </button></div>';
		$(params.selectedElement).parent().append(html);
...
var addDateInput = function(field_name){
