class AjaxController < ApplicationController
	...
	def get
		...
		if params[:kind] == 'developers_and_companies'
			developers = DeveloperVersioner.instance.current_versions_from_collection Developer.all
			result.concat(developers.collect! do |dev|
				{:value => {reference: {developer: dev.id.to_s}, name: dev.name}, :label => dev.name + ' (Developer)'}
			  end
			)
			companies = CompanyVersioner.instance.current_versions_from_collection Company.all
			result.concat(companies.collect! do |company|
				{:value => {reference: {company: company.id.to_s}, name: company.name}, :label => company.name + ' (Organisation)'}
			  end
			)
		end
